...
Només es documenten els elements públics.
*/
...
int main() {
    ...
    while (comando != "fin") {
        if (comando == "configurar_cluster" or comando == "cc") {
            ...
            cout << "#" << comando << endl;
            cl.imprimir_processadors_cluster();   //13
        }

        else if (comando == "imprimir_estructura_cluster" or comando == "iec") {
            cout << "#" << comando << endl;
            cl.imprimir_estructura_cluster();     //14
        }
        cin >> comando;
    }
}